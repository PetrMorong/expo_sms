...
const mapStateToProps = (store) => {
    return{
        historyCampaignDetail: store.historyCampaignDetail
    }
}
...
export default class CampaignDetail extends Component{
    ...
    componentWillMount(){
        AsyncStorage.getItem('filterHistoryDetail', (err, filter) => {
            if(filter){
    ...
    fetchData(newData){
        this.props.dispatch(fetch('history/get-history', {reducer: 'historyCampaignDetail', newData}, {from: this.state.from, to: this.state.to, limit: 15, offset: this.state.offset, filter: this.state.filter, campaign_id: this.props.campaign_id, username: null}))
    ...
    componentWillReceiveProps(nextProps){
        ...
        if(!nextProps.historyCampaignDetail.fetching){
    ...
    onEndReached(){
        if(this.props.historyCampaignDetail.data.result.total > this.state.offset && this.props.historyCampaignDetail.data.result.total > 15){
    ...
    handleSaveFilter(data){
        ...
        this.setState({offset: 0, showFilter: false, loadingFiltered: true, from: data.from, to: data.to, filter: filter}, () => {
            this.fetchData(true);
            AsyncStorage.setItem('filterHistoryDetail', JSON.stringify(filter))
        })
    ...
    clearFilterAll(){
        this.setState({
            ...
            this.fetchData(true);
            AsyncStorage.removeItem('filterHistoryDetail')
    ...
    render() {
        ...
        let loader;
        if(this.props.historyCampaignDetail.emptyData){
        ...
        return (
            <View style={{position: 'relative', flex: 1}}>
                <TouchableWithoutFeedback onPress={()=>{Actions.CampaignDashboard({campaign_id: this.props.campaign_id, year: this.props.year, month: this.props.month})}}>
                    <View style={{position: 'absolute', right: 60, top: 3, zIndex: 89, padding: 15}}>
                        <Icon name="visibility" size={30} style={{color: 'white'}}/>
                    </View>
                </TouchableWithoutFeedback>
                <DrawerLayout
                    drawerWidth={300}
                    ...
                    <View style={styles.container}>
                        ...
                        <FlatList
                            ...
                            style={{flex: 1}}
                            data={this.props.historyCampaignDetail.list}
                            ...
                            let itemRow;
                            if(item.type == 44){
                                itemRow = <TouchableOpacity onPress={()=> Actions.HistoryCampaignDetail({campaign_id: item.temp_id})} key={item.temp_id}>
                                            <View>
