...
export default class Vcard extends Component {
    ...
    selectAll(){
        ...
        if(this.state.selectedAll){
            ...
            this.setState({selected: this.props.campaign.vcard_list, selectedAll: true})
        }

    }

    render() {
        let vcardCount  = this.props.campaign.data.result.recipientsCount;

        let count;
        if(vcardCount.vcard > 0){
            count = <Text style={styles.count}>{vcardCount.vcard}</Text>
        }
        ...
        if(vcardCount.vcard == 0){
            emptyData = <View style={{height: window.height-100, width: window.width, justifyContent: 'center', alignItems: 'center'}}>
                <Icon name="contact-phone" size={80} style={{marginBottom: 30}}/>
                <Text style={{fontSize: 25, textAlign: 'center'}}>{_('For importing contacts from Vcard visit web application')}</Text>
            </View>
        }
            ...
            <DrawerLayout
                        ...
                        <View style={{flexDirection: 'row', alignItems: 'center', flex: 1, marginRight: 25, height: 60, paddingLeft: 15 }}>
                            ...
                            <View style={{height: 60, justifyContent: 'center'}}>
                                <View style={{ width: 180, flexDirection: 'row', alignItems: 'center'}}>
                                    <Text style={styles.screenName}>{_('Vcard') }</Text>
                                    {count}
                                </View>
                            </View>
                        </View>
                ...
                <View style={styles.container}>
                    ...
                    <FlatList
                        ...
                        style={{flex: 1}}
                        data={this.props.campaign.vcard_list}
...
module.exports = connect(mapStateToProps)(Vcard);
