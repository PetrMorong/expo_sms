...
const mapStateToProps = (store) => {
    return{
        scheduledDetail: store.scheduledDetail
    }
}
...
export default class Scheduled extends Component{
    ...
    fetchData(newData){
        ...
        this.props.dispatch(fetch('scheduled/get-scheduled-list', {reducer: 'scheduledDetail', newData}, {limit: 15, offset: this.state.offset, filter: {}, campaign_id: this.props.id}))

    }

    componentWillReceiveProps(nextProps){

        if(!nextProps.scheduledDetail.fetching){
    ...
    onEndReached(){
        if(this.props.scheduledDetail.data.result.total > this.state.offset && this.props.scheduledDetail.data.result.total > 15){
    ...
    deleteSelected(){
        this.props.dispatch(deleteListItem('scheduled/delete', {reducer: 'scheduledDetail'}, {sms_id: this.state.selected}, ()=>this.onDeleteSuccess()));
    }
    ...
    render() {
        ...
        let emptyData;
        if(this.props.scheduledDetail.emptyData){
            ...
            <DrawerLayout
                ...
                <View style={styles.container}>
                    ...
                    <FlatList
                        ...
                        style={{flex: 1}}
                        data={this.props.scheduledDetail.list}
                        ...
                        renderItem={({item}) => {
                            ...
                            let itemRow;
                            if(this.state.selected.indexOf(item.campaign_id) !== -1){
                                ...
                                itemRow = <TouchableWithoutFeedback onLongPress={(event) => this.select(item.campaign_id)} key={item.id}>
                                            <View>
