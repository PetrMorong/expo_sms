...
export default class InboxList extends Component{
    ...
    componentWillMount(){
        AsyncStorage.getItem('filterInbox', (err, filter) => {
    ...
    fetchData(newData){
        ...
        this.props.dispatch(fetch('inbox/load', {reducer: 'inboxList', newData}, {from: this.state.from, to: this.state.to, filter: this.state.filter, campaign_id: null, limit: 15, offset: this.state.offset}))
    ...
    componentWillReceiveProps(nextProps){
        ...
        if(!nextProps.inboxList.fetching){
    ...
    onEndReached(){
        if(this.props.inboxList.data.data.list.total > this.state.offset && this.props.inboxList.data.data.list.total > 15){
    ...
    handleSaveFilter(data){
        ...
        this.setState({offset: 0, showFilter: false, loadingFiltered: true, from: data.from, to: data.to, filter: filter}, () => {
            this.fetchData(true);
            AsyncStorage.setItem('filterInbox', JSON.stringify(filter))
        })
    ...
    clearFilterAll(){
        this.setState({
            ...
            this.fetchData(true);
            AsyncStorage.removeItem('filterInbox')
    ...
    render() {
        ...
        let emptyData;
        if(this.props.inboxList.emptyData){
            ...
            <DrawerLayout
                ...
                <View style={styles.container}>
                    ...
                    <FlatList
                        ...
                        style={{flex: 1}}
                        data={this.props.inboxList.list}
                        ...
                        renderItem={({item}) => {
                            ...
                            let itemRow;
                            if(item.campaign){
                                itemRow = <TouchableOpacity onPress={()=> Actions.InboxDetail({id: item.campaign_id})} key={item}>
                                <View>
