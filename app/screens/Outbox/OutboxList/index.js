...
export default class OutboxList extends Component{
    ...
    fetchData(newData){
        ...
        this.props.dispatch(fetch('outbox/get-outbox-list', {reducer: 'outboxList', newData}, {limit: 15, offset: this.state.offset, filter: {}, campaign_id: null}))
    ...
    componentWillReceiveProps(nextProps){
        ...
        if(!nextProps.outboxList.fetching){
    ...
    onEndReached(){
        if(this.props.outboxList.data.result.data.total > this.state.offset && this.props.outboxList.data.result.data.total > 15){
    ...
    deleteSelected(){
        this.props.dispatch(deleteListItem('outbox/delete', {reducer: 'outboxList'}, {campaign_id: this.state.selected}, ()=>this.onDeleteSuccess(), ()=>this.onDeleteError()));
    }
    ...
    render() {
        ...
        let emptyData;
        if(this.props.outboxList.emptyData){
            ...
            <DrawerLayout
                ...
                <View style={styles.container}>
                    ...
                    <FlatList
                        ...
                        style={{flex: 1}}
                        data={this.props.outboxList.list}
                        ...
                        renderItem={({item}) => {
                            ...
                            if(item.campaign_id == ""){
                                ...
                                itemRow = <TouchableWithoutFeedback key={item.id} onLongPress={(event) => this.select(item.sms_id, item.campaign_id)}>
                                    <View>
                                ...
                                itemRow = <TouchableWithoutFeedback onPress={()=>Actions.OutboxDetail({id: item.campaign_id})} onLongPress={(event) => this.select(item.sms_id, item.campaign_id)} key={item.id}>
                                            <View>
                                                <View style={[styles.itemWrap, background]}>
                                                    <GetProduct product={item.product}/>
                                                    <View style={{flex: 1, marginLeft: -5}}>
